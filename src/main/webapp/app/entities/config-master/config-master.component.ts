...
import { IConfigMaster } from '../../shared/model/config-master.model';
...
export class ConfigMasterComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.configMasterService
      ...
      .subscribe(
        (res: HttpResponse<IConfigMaster[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IConfigMaster): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInConfigMasters(): void {
    this.eventSubscriber = this.eventManager.subscribe('configMasterListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IConfigMaster[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
