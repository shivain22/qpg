...
import com.qpg.converter.internal.documents.Image;
...
import static com.qpg.converter.internal.docx.ReadResult.*;
import static com.qpg.converter.internal.docx.Uris.uriToZipEntryName;
...
import static com.qpg.converter.internal.util.Sets.set;

class StatefulBodyXmlReader {
    private static final Set<String> IMAGE_TYPES_SUPPORTED_BY_BROWSERS = Sets.set(
        ...
        return ReadResult.map(
            ...
            (style, children) -> {
                ...
                if (hyperlinkHref.isPresent()) {
                    children = Lists.list(Hyperlink.href(hyperlinkHref.get(), Optional.empty(), children));
                }
        ...
        Function<String, Optional<Style>> findStyleById)
    {
        ...
        if (style.isPresent()) {
            ...
            return new InternalResult<>(
                ...
                Lists.list(styleType + " style with ID " + styleId + " was referenced but not defined in the document"));
        }
    ...
    private ReadResult calculateRowspans(List<DocumentElement> rows) {
        ...
        for (int rowIndex = 0; rowIndex < rows.size(); rowIndex += 1) {
            ...
            for (int cellIndex = 0; cellIndex < row.getChildren().size(); cellIndex += 1) {
                ...
                Optional<Map.Entry<Integer, Integer>> spanningCell = Maps.lookup(lastCellForColumn, columnIndex);
            ...
            for (int cellIndex = 0; cellIndex < row.getChildren().size(); cellIndex += 1) {
                ...
                if (!merged.contains(position)) {
                    mergedCells.add(new TableCell(
                        Maps.lookup(rowspans, position).get(),
                        cell.colspan,
                        cell.children
                    ));
                }
    ...
    private ReadResult readImagedata(XmlElement element) {
        return element.getAttributeOrNone("r:id")
            .map(relationshipId -> {
                ...
                return readImage(imagePath, title, () -> Archives.getInputStream(file, imagePath),Optional.of(""));
            })
    ...
    private ReadResult readBlip(XmlElement blip, Optional<String> altText) {
        ...
        if (embedRelationshipId.isPresent()) {
            ...
            return readImage(imagePath, altText, () -> Archives.getInputStream(file, imagePath),Optional.of(""));
        } else if (linkRelationshipId.isPresent()) {
            String imagePath = relationships.findTargetByRelationshipId(linkRelationshipId.get());
            return readImage(imagePath, altText, () -> fileReader.getInputStream(imagePath),Optional.of(""));
        } else {
            // TODO: emit warning
            return ReadResult.EMPTY_SUCCESS;
        }
    ...
    private ReadResult readImage(String imagePath, Optional<String> altText, InputStreamSupplier open,Optional<String> imageLocation) {
        Optional<String> contentType = contentTypes.findContentType(imagePath);
        Image image = new Image(altText, contentType, open,Optional.of(imagePath),Optional.of(""));
