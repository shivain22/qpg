...
import static com.qpg.converter.internal.util.Iterables.stream;
...
public class Lists {
    ...
    public static <T, R> List<R> eagerFlatMap(Iterable<T> iterable, Function<T, Iterable<R>> function) {
        return stream(iterable).flatMap(element -> stream(function.apply(element))).collect(Collectors.toList());
    }
